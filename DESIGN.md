...
- **Card** (`.check-card`): top hairline; kicker row (mono: SECTION · date); headline = the key finding (display serif, link); **claim line** (`.check-card__claim`, mono `--fs-meta` in `--ink-60`): uppercase `CLAIM:` label + quoted claim (~80 chars, truncated) + `.bs-pill` inline at line end — the pill never sits alone under the finding. Variants: `--row` (section blocks/feeds: same anatomy, tighter); `--headline` (the Fresh Checks rail: kicker + linked finding only, unclamped — a headline stack; no claim, no pill — only the finding may stand alone).
